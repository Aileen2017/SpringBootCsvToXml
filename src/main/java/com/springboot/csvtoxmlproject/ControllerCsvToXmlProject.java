...
public class ControllerCsvToXmlProject {
	...
	public String uploadCsvFormReturnDownload(Model model) {
		//NullPointerException nperror = new NullPointerException();
		//throw nperror;
		 return "uploadCsvTemplate";
	}

	@PostMapping("/uploadCsv_return_download")
	public ResponseEntity uploadCsvSubmitReturnDownload(@RequestParam("file1") MultipartFile file, Model model)
			throws Exception {
		if (file.isEmpty())
		...
		return ResponseEntity.ok().header("Content-Disposition", "attachment;filename=" + fileName + ".xml")
				.contentLength(outputStream.toString().length()).body(outputStream.toString().getBytes());

	}

	@PostMapping("/uploadCsv_with_headers_return_download")
	public ResponseEntity uploadCsvWithHeadersSubmit(@RequestParam("hiddenText") String hiddenText,
			@RequestParam("file1") MultipartFile file, Model model) throws Exception {
		if (file.isEmpty())
			throw new EmptyFileException();
	...
	// ExceptionHandler error handling: Return error in user defined web page myerror.html, but not with customised error
	// messages(eg. ex, url, defined in mav 
